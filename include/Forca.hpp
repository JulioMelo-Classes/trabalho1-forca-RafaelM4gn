...
private:
	...
	struct Scores_base
	{
		std::string s_dificuldade;
		std::string s_nome;
		std::vector<std::string> s_palavras;
		int s_pontos;
	};

	std::vector<Scores_base> m_scores; //<! palavras e sua ocorrência no Corpus
	...
	std::vector<std::string> m_palavras_acertadas; //<! container “Palavras acertadas”
...
public:
	...
	/**
	 * Retorna e reseta a quantidade de pontos.
	 * @return a quantidade de pontos conquistados.
	 */
	int get_reset_pontos();

	/**
	 * Retorna a dificuldade do jogo.
	 * @return a nivel de dificuldade.
	 */
	std::string get_dificuldade();

	/**
	 * Retorna as palavras acertadas pelo jogador.
	 * @return as palavras acertadas.
	 */
	std::vector<std::string> get_palavras_acertadas();
	/**
	 * Imprime o boneco da forca baseado em m_tentativas_restantes.
	 */
	void imprimirForca();
	/**
	 * Imprime a tabela de pontuacao baseado em m_scores.
	 */
	void imprimirScores();
};
